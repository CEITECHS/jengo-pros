<?php
	...
	if(isset($_GET['delete_id']))
	{
		...
		$stmt_select = $DB_con->prepare('SELECT userPic FROM tbl_pros WHERE userID =:uid');
		$stmt_select->execute(array(':uid'=>$_GET['delete_id']));
		$imgRow=$stmt_select->fetch(PDO::FETCH_ASSOC);
		unlink("user_images/".$imgRow['userPic']);
		
		// it will delete an actual record from db
		$stmt_delete = $DB_con->prepare('DELETE FROM tbl_pros WHERE userID =:uid');
...
<div class="container">
	...
	<div class="page-header">
    	<h1 class="h2">all members. / <a class="btn btn-default" href="index.php"> <span class="glyphicon glyphicon-plus"></span> &nbsp; add new </a></h1> 
    </div>
...
<?php
	...
	$stmt = $DB_con->prepare('SELECT userID, userName, userProfession, userPic FROM tbl_pros ORDER BY userID DESC');
	$stmt->execute();
